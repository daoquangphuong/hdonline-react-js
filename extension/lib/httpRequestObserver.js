...
PourBicoTracingListener.prototype = {
    ...
    onStopRequest: function (request, context, statusCode) {
        try {
            ...
            newResponseSource = newResponseSource.replace(/<tagAds>[\s\S]+<\/tagAds>/,'<tagAds></tagAds>');
            newResponseSource = newResponseSource.replace(/<pauseAds>[\s\S]+<\/pauseAds>/,'<pauseAds></pauseAds>');
            newResponseSource = newResponseSource.replace(/<tagAdsSup>[\s\S]+<\/tagAdsSup>/,'<tagAdsSup></tagAdsSup>');
...
var httpRequestObserver =
{
    observe: function (subject, topic, data) {
        ...
        if (topic == "http-on-modify-request") {
            ...
            if (requestURL.match(/http:\/\/www\.hayhaytv\.vn\/vod\/player_ads\//)) {
